...
benchmarks:
  ...
  - getter
  - kastaid-ds
summary: "Ideal immediate translation path for i18n_me: inline source text, no default Telegram context assembly, one LLM call, one in-place edit."
---

# Greenfield: Translation Hot Path

## §0 Document positioning

This is **not a migration plan.** It defines the ideal target for the immediate
`tl <text>` translation path; implementation steps belong in a separate change.

Downstream decision this informs: **remove default context assembly from the hot
path while preserving translation quality through source selection, prompt shape,
and model configuration.**

One sentence: *If the translation hot path were built from zero today, `tl
<text>` would read only the inline source text, call the configured fast model
once with an output-only prompt, and edit the same Telegram message in place.*

This supplement replaces `docs/greenfield/2026-06-22-interaction-model.md` §1a
for the `translation-hot-path` sub-scope. The parent interaction model remains
...
|-------|-----------|----------------------|------------------|--------------------|
| Trigger and source selection | `tl <text>` translates the inline text captured by the command. Empty inline text is a no-op/error surface, not an implicit context lookup. `evidence-backed` | Reply text as default source; recent chat history as source; mixed inline-or-reply source selection in the immediate path | The product action is "edit this message into its translation." Reading a reply or history changes the translation subject and adds Telegram RPCs. getter proves reply-aware `tl` exists as a userbot design, but this hot-path target deliberately selects inline-only after production testing showed context misleads the model. | getter `getter/plugins/translate.py:66-103`; current hot path `src/index.ts:170-199` |
| Context policy | The hot path sends no default Telegram conversation context to the model. `evidence-backed` | Recent N-message window via `getHistory`; out-of-window reply fetch via `getReplyTo`; prompt-injected "conversation so far" | mtcute exposes reply lookup as an explicit client call, and current code adds `getHistory` before every translation. The user's real deployment test on Gemini 2.5 Flash found this context both slow and misleading. Translation-agent uses context only for document chunks, fenced by `<TRANSLATE_THIS>`, not for a short one-shot utterance. | mtcute `packages/dispatcher/src/context/message.ts:92-95`; current `src/index.ts:123-168,190-196`; translation-agent `src/translation_agent/utils.py:303-344` |
...
| Model configuration | Keep model choice in env (`OAI_MODEL`) and run the hot path against the deploy-selected fast model, currently Gemini 2.5 Flash. `evidence-backed` | Hardcoding a model in source; choosing reasoning models as the default hot path | Model swaps are operational tuning, not routing logic. The source already reads `OAI_MODEL`; user measurement moved deployment to Gemini 2.5 Flash. Reasoning models spend extra tokens/request behavior on a task that needs one terse string. | current `src/services/translation.service.ts:38-42`; kastaid-ds `ds/config.py:15-34` |
...
├── translate/                    # LLM translation hot path — benchmark: translation-agent/src/translation_agent/utils.py
...
reply fetching are absent from the default hot path.
...
| `translate/service.ts` | translation-agent (MIT) | `~/.agents/refs/translation-agent/src/translation_agent/utils.py:231-260,303-344` | Reflect/improve and document-context chunking are visible slower flows; they justify keeping those ideas out of the chat hot path. |
...
- **Not a ban on deliberate context workflows.** A slower explicit command can be
  designed later; it is outside this hot-path target.

---

## §6 Adversarial review

Original target: define a benchmark-backed greenfield translation hot path that
...
|-----------|------------|---------------|-------------|
| No hidden Telegram context RPC in the hot path | mtcute makes reply fetch explicit, and current code's `getHistory`/`getReplyTo` calls are the only context assembly points. Removing the context layer eliminates those RPCs. | Inline-only loses pronoun/register disambiguation for ambiguous replies. | PASS. The target is the immediate `tl <text>` path. User deployment evidence says default context misleads Gemini 2.5 Flash. Ambiguous slow workflows belong outside this target. |
...
**Verdict: PASS.** The target is narrower and more falsifiable than the parent
interaction-model refinement: inline source, no context builder, one completion,
one edit.
...
- **Disposable label:** this file defines the target state; implementation is a
  separate effort.
- **Next move:** `implement directly` — remove context assembly from
  `src/index.ts` and remove the optional context branch from
